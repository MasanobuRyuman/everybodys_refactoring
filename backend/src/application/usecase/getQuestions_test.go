...
func Test_getQuestions(t *testing.T) {
	...
	getQuestoinInteractor := new(usecase.GetQuestionInteractor)
	getQuestoinInteractor.QuestionRepository = questionRepository
	question, err := getQuestoinInteractor.GetQuestions(questions[0].Id)
	if err != nil {
		t.Error(err)
	}
